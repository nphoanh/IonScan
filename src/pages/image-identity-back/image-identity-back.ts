...
export class ImageIdentityBackPage {
	...
	saveImage(){
		...
		if (this.data != null) { 
			let nameEmail = this.data.substr(0,this.data.lastIndexOf('@'));
			let nameDB = nameEmail + '.db';
			let folderPath = this.file.externalRootDirectory + 'IonScan' + '/' + 'Identity' + '.' + nameEmail;                                    this.sqlite.create({
				name: nameDB,
				location: 'default'
			}).then((db: SQLiteObject) => {                
				db.executeSql('INSERT INTO image VALUES (NULL,?,?,?,?,?,"1")', [this.image.name,this.image.date,folderPath,src,this.image.type]).then(res => {
					this.savebase64AsFile(folderPath, nameFile, base, this.image.type); 
					this.navCtrl.push(InfoIdentityPage,{pictureFront:src}); 
				}).catch(e => {
